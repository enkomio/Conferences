...
namespace dnlib.DotNet.Writer {
	...
	public sealed class MethodBodyWriter : MethodBodyWriterBase {
		...
		public void Write() {
            ...
            // MODDED
            if (this.cilBody.RawBody != null && this.cilBody.RawBody.Count > 0)
            {
                codeSize = (uint)this.cilBody.RawBody.Count;
            }
        ...
        // MODDED
        public uint WriteInstructions(BinaryWriter writer)
        {
            if (this.cilBody.RawBody != null && this.cilBody.RawBody.Count > 0)
            {
                writer.Write(this.cilBody.RawBody.ToArray());
                return (uint)this.cilBody.RawBody.Count;
            }

            return base.WriteInstructions(writer);
        }
